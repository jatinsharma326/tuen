...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setOk(true); setTimeout(() => setOk(false), 1500); }} className="rounded-md p-1.5 text-white/30 hover:text-white/70 hover:bg-white/[0.03] transition-colors">
...
function CodeBlock({ lang, code }: { lang: string; code: string }) {
  return (
    <div className="overflow-hidden rounded-xl border border-white/[0.05]">
      ...
      <pre className="overflow-x-auto bg-[#12121a]/30 p-5 font-mono text-[12px] leading-relaxed text-white/40">
        {code}
      </pre>
    </div>
  );
...
export default function DocsPage() {
      ...
      <div className="flex gap-1 mb-3">
        {Object.keys(tabs).map((lang) => (
          <button key={lang} onClick={() => setLang(id, lang)} className={cn(
            ...
            getTab(id) === lang ? "bg-white/[0.03] text-white ring-1 ring-border-default" : "text-white/30 hover:text-white/70"
          )}>
            {lang}
          </button>
        ))}
    ...
    <div className="mx-auto max-w-6xl px-5 py-24">
      <div className="lg:grid lg:grid-cols-[220px_1fr] lg:gap-14">
        ...
        <nav className="hidden lg:block sticky top-24 self-start">
          <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-white/30 mb-3">On this page</p>
          {SECTIONS.map((s) => (
            <a key={s.id} href={`#${s.id}`} className="flex items-center gap-1.5 py-1.5 text-[13px] text-white/30 hover:text-white/70 transition-colors">
        ...
        <div className="space-y-20">
          ...
          <div>
            ...
            <h1 className="mt-2 font-display text-[32px] font-extrabold tracking-tight text-white">tuen API Documentation</h1>
            <p className="mt-3 text-[14px] text-white/40 leading-relaxed max-w-2xl">
          ...
          <section id="quickstart" className="space-y-6">
            <h2 className="font-display text-[22px] font-bold tracking-tight text-white">Quick Start</h2>
            ...
            <div className="space-y-6">
              <Step num={1} title="Get your API key">
                <p className="text-[13px] text-white/40">
                  Sign in and go to <a href="/dashboard/api-keys" className="text-accent hover:underline">API Keys</a>. Copy your key — it starts with <code className="text-white/70">tuen_sk_</code>.
                </p>
              </Step>
              <Step num={2} title="Make your first request">
                <p className="text-[13px] text-white/40 mb-3">
                  Send a POST request with your key in the <code className="text-white/70">Authorization</code> header.
                </p>
  ...
  -d '{"prompt": "a futuristic city at sunset"}'`} />
              ...
              <Step num={3} title="Use the response">
                <p className="text-[13px] text-white/40">
                  The API returns a JSON object with the result URL. For images: <code className="text-white/70">image_url</code>. For audio: <code className="text-white/70">audio_url</code>.
                </p>
              </Step>
            </div>
          </section>
          ...
          <section id="auth" className="space-y-4">
            <h2 className="font-display text-[22px] font-bold tracking-tight text-white">Authentication</h2>
            <p className="text-[13px] text-white/40">
              All API requests require an <code className="text-white/70">Authorization</code> header with your API key.
            </p>
          ...
          <section id="image" className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <Badge>POST</Badge>
                  <code className="font-mono text-[13px] text-white">/api/services/image_gen</code>
                  <span className="badge-premium bg-white/[0.03] text-white/30 border-white/[0.08]">1 request</span>
                </div>
              </div>
            </div>
            <p className="text-[13px] text-white/40">
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-[0.1em] text-white/30">Request body</h3>
            <div className="overflow-hidden rounded-xl border border-white/[0.05] text-[13px]">
              <div className="grid grid-cols-[140px_80px_1fr] gap-x-4 border-b border-white/[0.05] bg-[#12121a]/60 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.1em] text-white/30">
              ...
              {[
                ...
                <div key={p.name} className={`grid grid-cols-[140px_80px_1fr] gap-x-4 px-5 py-3 ${i < 3 ? "border-b border-white/[0.05]" : ""}`}>
                  <code className="text-white">{p.name}{p.req ? " *" : ""}</code>
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-[0.1em] text-white/30">Example request</h3>
...
console.log(image_url);`,
            ...
            <h3 className="text-[13px] font-bold uppercase tracking-[0.1em] text-white/30">Response</h3>
            <CodeBlock lang="json" code={`{
  "image_url": "https://cdn.example.com/output/image.webp",
  "success": true
}`} />

            <div className="rounded-xl border border-white/[0.05] bg-[#12121a]/30 px-5 py-4">
              <p className="text-[12px] text-white/70">
                <strong>Tip:</strong> Use <code className="text-white">aspect_ratio: "1:1"</code> for profile pictures,
                <code className="text-white">"9:16"</code> for mobile wallpapers, and <code className="text-white">"16:9"</code> for desktop backgrounds.
              </p>
            </div>
          </section>
          ...
          <section id="tts" className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <Badge>POST</Badge>
                  <code className="font-mono text-[13px] text-white">/api/services/tts</code>
                  <span className="badge-premium bg-white/[0.03] text-white/30 border-white/[0.08]">1 request</span>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-white/[0.05] text-[13px]">
              ...
              {[
                ...
                <div key={p.name} className={`grid grid-cols-[140px_80px_1fr] gap-x-4 px-5 py-3 ${i < 2 ? "border-b border-white/[0.05]" : ""}`}>
                  <code className="text-white">{p.name}{p.req ? " *" : ""}</code>
                  <span className="text-white/30">{p.type}</span>
                  <span className="text-white/40">{p.desc}</span>
                </div>
          ...
          <section id="transcribe" className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <Badge>POST</Badge>
                  <code className="font-mono text-[13px] text-white">/api/services/transcribe</code>
                  <span className="badge-premium bg-white/[0.03] text-white/30 border-white/[0.08]">1 request</span>
                </div>
              </div>
            </div>
  ...
  "success": true
          ...
          <section id="n8n" className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-display text-[22px] font-bold tracking-tight text-white">n8n Integration Guide</h2>
            </div>
            <p className="text-[13px] text-white/40">
            ...
            <div className="space-y-6">
              ...
              <Step num={2} title="Configure the request">
                <div className="space-y-2">
                  <p className="text-[13px] text-white/40">Set these fields:</p>
                  <div className="overflow-hidden rounded-xl border border-white/[0.05] text-[13px]">
                    ...
                    {[
                      ...
                      <div key={r.field} className={`grid grid-cols-[140px_1fr] gap-x-4 px-5 py-2.5 ${i < 4 ? "border-b border-white/[0.05]" : ""}`}>
                        ...
                        <code className="text-white/40">{r.value}</code>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <Step num={3} title="Set the JSON body">
                <p className="text-[13px] text-white/40 mb-2">Switch the body type to <strong>JSON</strong> and paste:</p>
                <CodeBlock lang="json" code={`{
  "prompt": "{{ $json.prompt }}",
  "aspect_ratio": "1:1"
}`} />
              ...
              <Step num={4} title="Use the output">
                <p className="text-[13px] text-white/40">
                  The node returns <code className="text-white/70">image_url</code> (or <code className="text-white/70">audio_url</code> / <code className="text-white/70">text</code>).
                  Access it with <code className="text-white/70">{"{{ $json.image_url }}"}</code> in the next node.
                </p>
              </Step>
            </div>
          ...
          <section id="errors" className="space-y-5">
            <h2 className="font-display text-[22px] font-bold tracking-tight text-white">Errors & Rate Limits</h2>
            <p className="text-[13px] text-white/40">
              All errors return a JSON body with an <code className="text-white/70">error</code> field.
              Failed requests are not counted against your limits — you never pay for errors.
            </p>

            <h3 className="text-[13px] font-bold uppercase tracking-[0.1em] text-white/30">Error codes</h3>
            <div className="overflow-hidden rounded-xl border border-white/[0.05] text-[13px]">
              <div className="grid grid-cols-[80px_1fr] gap-x-4 border-b border-white/[0.05] bg-[#12121a]/60 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.1em] text-white/30">
              ...
              {[
                ...
                <div key={e.code} className="grid grid-cols-[80px_1fr] gap-x-4 border-b border-white/[0.05] px-5 py-3 last:border-0">
                  <code className="text-white">{e.code}</code>
                  <span className="text-white/40">{e.desc}</span>
                </div>
              ))}
            </div>

            <h3 className="text-[13px] font-bold uppercase tracking-[0.1em] text-white/30">Usage limits</h3>
            <div className="overflow-hidden rounded-xl border border-white/[0.05] text-[13px]">
              <div className="grid grid-cols-4 gap-x-4 border-b border-white/[0.05] bg-[#12121a]/60 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.1em] text-white/30">
              ...
              {[
                ...
                <div key={p.service} className="grid grid-cols-4 gap-x-4 border-b border-white/[0.05] px-5 py-3 last:border-0">
                  <span className="font-medium text-white">{p.service}</span>
                  <span className="text-white/30">{p.daily}</span>
                  <span className="text-white/30">3,000 total</span>
                  <span className="text-white/30">10,000 total</span>
                </div>
              ))}
            </div>
